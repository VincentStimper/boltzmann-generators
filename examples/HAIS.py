...
target_dist = bg.distributions.TransformedBoltzmann(
...
for i in range(1000):
    print(i)
    samples, logweights = hais.sample(1000)
    samples = samples.detach().numpy()
    logweights = logweights.detach().numpy()
    np.save('saved_data/1707_hais/samples_{}'.format(i), samples)
    np.save('saved_data/1707_hais/logweights_{}'.format(i), logweights)
